...
public class TC_170_FlightSearchAndSearchFunctionality extends BaseClass {
    ...
    public void myTest(Map<String, String> excelData) throws InterruptedException, IOException {
     ...
     //  Map<String, String> excelData = getDataFromExcel.getExcelData("TC_170");
       ...
       String numberofstops1 = excelData.get("NumberOfStops1");
        ...
        tripgainresultspage.validateFlightsResults(Log, screenShots);
         Thread.sleep(3000);
         //tripgainresultspage.validateArrivalTimeSortFunctionalityOnResultScreen(Log, screenShots, "ARRIVAL");
       //Function to validate Duration Functionality
         tripgainresultspage.clickOnFiltersOnResultScreen(filtername);
         tripgainresultspage.validateDurationTimeSortFunctionalityOnResultScreen(Log, screenShots, order1);
         Thread.sleep(5000);
         tripgainresultspage.clickOnFiltersOnResultScreen(filtername);
         tripgainresultspage.validateDurationTimeSortFunctionalityOnResultScreen(Log, screenShots, order2);
         Thread.sleep(5000);
         tripgainresultspage.clickOnCurrencyDropDown();
         Thread.sleep(4000);
    		tripgainresultspage.selectCurrencyDropDownValues(driver, currencyCode2);
   		//tripgainresultspage.validateCurrencyOnResultScreen("IRR", Log, screenShots);		
   		tripgainresultspage.validateCurrency(currencyCode2, Log, screenShots);
        Thread.sleep(3000); 

        Thread.sleep(3000); 
  tripgainresultspage.selectOnWardDepartTime(times);
  Thread.sleep(3000); 
 // tripgainresultspage.validateOnwardDepartureTimeIsSelected(Log, screenShots, "00 - 06");
  tripgainresultspage.validateFlightsDepartureTimeOnResultScreen(flightStartHour, flightStartMinute, flightEndHour, flightEndMinute, Log, screenShots);
