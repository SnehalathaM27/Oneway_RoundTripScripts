...
public class TC_181_VerifyAirportChange extends BaseClass {
    ...
    public void myTest(Map<String, String> excelData) throws InterruptedException, IOException {
       ...
       String fareTypeArg = excelData.get("FareTypeArg");
...
tripgainhomepage.searchFlightsOnHomePage(origin, destination,  fromDate, fromMonthYear, travelClass, adults, Log, screenShots);
      ...
      // String [] userInput= trs.userEnterData();
        ...
        Thread.sleep(3000); 
      //  tripgainresultspage.clickOnSelectFlightBasedOnIndex(2);
        Thread.sleep(3000); 
        tripgainresultspage.storeLocationCodesFromResultPage(Log, screenShots, index);

      //tripgainresultspage.getLocationCodesFromResultPage(Log, screenShots, 2);
        Thread.sleep(3000); 
        tripgainresultspage.selectFaretypeOneway(fareTypeArg, Log, screenShots);
        Thread.sleep(4000); 
       
        tripgainresultspage.getLocationCodesFromChangePopup(Log, screenShots);
        Thread.sleep(3000); 
        tripgainresultspage.clickYesContinueButton();
        Thread.sleep(3000); 
        tripgainresultspage.reasonForSelectionPopUp();
        Thread.sleep(3000);
       // tripgainresultspage.validateFlightDetailsOnBookingPage(resultData, Log, screenShots);

       tripgainresultspage.validateBookingPageLocationAgainstResult(Log, screenShots);
